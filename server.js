...
function log(level, message, meta = {}) {
  if (logLevels[level] <= logLevels[LOG_LEVEL]) {
    console.log(`[${new Date().toISOString()}] [${level.toUpperCase()}] ${message}`, JSON.stringify(meta))
  }
}
...
startServer({
  ...
  log('info', 'Server started', { port: currentPort, hostname })
  ...
  healthServer.listen(currentPort + 1, hostname, () => {
    log('info', 'Health check server running', { port: currentPort + 1 })
  })
}).catch((err) => {
